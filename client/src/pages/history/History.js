...
const History = () => {
  ...
  useEffect(() => {
    axios.get(API.GET_DEVICES_LIST).then((result) => {
  ...
  const getHistoryData = (e) => {
    ...
    axios.get(url).then((result) => {
      ...
      result.data.forEach((element) => {
        const kst_time = moment(element.created_at)
          .utcOffset("+09:00")
          .format("YYYY-MM-DD HH:mm:ss");
        temperatureDataList.push(createData(kst_time, element.temperature));
        humidityDataList.push(createData(kst_time, element.humid));
      });
      ...
      <Grid container spacing={3}>
        <Grid item lg={6}>
          ...
          >
            ...
            <Select value={deviceIndex} onChange={handleDeviceChange}>
            ...
            <Button variant="contained" onClick={getHistoryData}>
              {" "}
              조회{" "}
            </Button>
          </Card>
